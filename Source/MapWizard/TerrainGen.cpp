...
void TerrainGen::create_terrain(TArray<TSharedPtr<Node>>& roads_, TArray<TSharedPtr<District>>& figures_array_, TArray<TSharedPtr<Street>>& streets_array_,
                                TArray<TSharedPtr<Street>>& segments_array_, TArray<TSharedPtr<District>>& river_figure_,
                                TArray<TSharedPtr<Node>>& map_borders_array_, TArray<FVector>& debug_points_array_)
{
	...
	for (auto& cd : custom_distr_nodes)
	{
		custom_districts.Add(
			TTuple<FVector, district_type>((cd.node1->get_FVector() + cd.node2->get_FVector() + cd.node3->get_FVector()) / 3, district_type::water));
	}
	...
	if (central_node.IsValid())
	{
		...
		create_circle(central_point, 200, district_type::royal, point_type::main_road, 25);
	}
	for (auto t : towers)
	{
		create_circle(t, tower_radius, district_type::tower, point_type::wall, 8);
	}
	process_streets(roads, streets_array, point_type::wall, true);
                                              ...
                                              const TSharedPtr<Node>& start_point_right)
{
	...
	auto left_map_border_intersect = AllGeometry::is_intersect_array(end_point->get_FVector(), end_point_left->get_FVector(), map_borders_array, false);
	...
	TSharedPtr<Node> end_point_right = MakeShared<Node>(AllGeometry::create_segment_at_angle(
		start_point->get_FVector(), end_point->get_FVector(), end_point->get_FVector(), 90, av_river_length / 2));
	auto right_map_border_intersect = AllGeometry::is_intersect_array(end_point->get_FVector(), end_point_right->get_FVector(), map_borders_array, false);
	...
	for (int i = 1; i <= dist_times; i++)
	{
		...
		custom_distr_nodes.Add(CustomDistrNodes(node_ptr_left, node_ptr_right, old_node_right));
...
bool TerrainGen::create_guiding_road_segment(const TSharedPtr<Node>& start_point, const TSharedPtr<Node>& end_point, bool is_through_river, point_type road_type)
{
	...
	do
	{
		...
		auto inter_point1 = AllGeometry::is_intersect_array(local_start_point, end_point, river, false);
		if (inter_point1.IsSet())
		{
			...
			for (int i = 10; ; i += 5)
			{
				...
				{
					auto inter = AllGeometry::is_intersect_array(point_end, inter_point1->Key, river, false);
					if (inter.IsSet())
					{
			...
			real_bridge_end = AllGeometry::create_segment_at_angle(local_end_point->get_FVector(), next_start_point->get_FVector(),
			...
			next_start_point->set_FVector(real_bridge_end);
			create_segment(roads, local_end_point, next_start_point, true, road_type, 5000);
			// if (road_type != wall)
			// {
	...
	for (auto node : nodes)
	{
		for (auto lconn : node->conn)
		{
			...
			else
			{
				...
				if (
					(general_angle / (figure_array->Num() - 1) < 180/* && !is_river*/)
...
void TerrainGen::get_river_figure()
{
	...
	river_figures.RemoveAll(
		[&](TSharedPtr<District>& A)
		{
			...
			{
				if (point.Value == district_type::water && A->is_point_in_figure(point.Key))
				{
					is_in = true;
					break;
				}
...
{
	districts.Sort([this](TSharedPtr<District> Item1, TSharedPtr<District> Item2) { return Item1->area > Item2->area; });
	double royal_area = 0;
	...
	for (auto& b : districts)
	{
		...
		if (b->get_type() == district_type::unknown)
		{
			...
			for (int i = 2; i < fig_size + 2; i++)
			{
				...
				if (is_point_in_river(center_of_dist) && b->is_point_in_figure(center_of_dist))
				{
					in_river = true;
					break;
				}
		...
		if (b->get_type() == district_type::unknown)
		{
			...
			for (auto p : b->figure)
			{
				if (p->get_point()->districts_nearby.Contains(district_type::royal))
				{
					is_near_royal = true;
				}
				if (p->get_point()->districts_nearby.Contains(district_type::water))
				{
					is_near_dock = true;
				}
			}
	...
	if (district->get_type() == district_type::luxury)
	{
		...
		for (int i = 1; i <= figure_num; i++)
		{
			if (district->self_figure[i - 1].districts_nearby.Contains(district_type::royal) &&
				district->self_figure[i % figure_num].districts_nearby.Contains(district_type::royal))
			{
...
void TerrainGen::process_streets(TArray<TSharedPtr<Node>> nodes, TArray<TSharedPtr<Street>>& fig_array, point_type type, bool is_persistent)
{
	...
	for (auto node : nodes)
	{
		for (auto nconn : node->conn)
		{
			...
			do
			{
				counter++;
				is_next_found = false;
				third_point = get_next_road_node(first_point, second_point, type_used, is_persistent);
				if (third_point)
				{
...
TArray<TSharedPtr<Street>> TerrainGen::process_segments(TArray<TSharedPtr<Street>>& fig_array)
{
	TArray<TSharedPtr<Street>> segments{};
	for (auto& street: fig_array )
	{
		if (street->street_vertices.Num()<2) break;
		TSharedPtr<Street> cur_street(MakeShared<Street>(TArray<TSharedPtr<Node>>()));
		cur_street->street_vertices.Add(street->street_vertices[0]);
		for (int i=1;i<street->street_vertices.Num(); i++)
		{
			if (street->street_vertices[i]->conn.Num()>2 || i==street->street_vertices.Num()-1)
			{
				cur_street->street_vertices.Add(street->street_vertices[i]);
				cur_street->type = street->type;
				segments.Add(cur_street);
				cur_street = TSharedPtr<Street>(MakeShared<Street>(TArray<TSharedPtr<Node>>()));
			}
			auto next = street->street_vertices[i-1]->get_next_point(street->street_vertices[i]->get_point());
			auto prev = street->street_vertices[i-1]->get_prev_point(street->street_vertices[i]->get_point());
			if (next.IsSet())
			{
...
TSharedPtr<Node> TerrainGen::get_next_road_node(TSharedPtr<Node> first_point, TSharedPtr<Node> second_point, point_type type, bool is_persistent)
{
	...
	for (auto second_node_conn : second_point->conn)
	{
		...
		auto angle_before_abs = AllGeometry::calculate_angle(first_point->get_FVector(), second_point->get_FVector(), third_point->get_FVector(), false);
		auto angle = FMath::Abs(180 - angle_before_abs);
		if (angle < 25 || (angle_before_abs > 90 && second_point->conn.Num() == 2))
		{
