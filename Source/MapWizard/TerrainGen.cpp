...
// void TerrainGen::del_conn(const TSharedPtr<Node>& node1, const TSharedPtr<Node>& node2)
// {
// 	if (node1->get_next_point(node2->get_point()).IsSet())
// 	{
// 		node1->del_connection(node2);
// 	}
// 	if (node2->get_next_point(node1->get_point()).IsSet())
// 	{
// 		node2->del_connection(node1);
// 	}
// }
                                ...
                                TArray<FVector>& debug2_points_array_)
{
	...
	process_streets(road_nodes, streets_array, EPointType::MainRoad, false);
                                              ...
                                              TArray<TSharedPtr<Node>>& river)
{
	...
	end_point_left->set_type(EPointType::River);
	end_point_right->set_type(EPointType::River);
	...
	for (int i = 1; i <= dist_times; i++)
	{
		...
		TSharedPtr<Node> node_ptr_right =
			MakeShared<Node>(start_point_right->get_FVector() +
				((end_point_right->get_FVector() - start_point_right->
					get_FVector()) / dist_times * i));
		node_ptr_left->set_type(EPointType::River);
		node_ptr_right->set_type(EPointType::River);
		if (i != dist_times)
		{
			create_segment(river, node_ptr_left, old_node_left, true,
			               EPointType::River, map_params.max_river_length);
		...
		// custom_districts.Add(TTuple<FVector, Edistrict_type>((
...
void TerrainGen::create_guiding_roads(TArray<WeightedPoint>& weighted_points, const TArray<TSharedPtr<Node>>& river)
{
	...
	{
		r->set_type(EPointType::MainRoad);
	}
		...
		for (int i = 0; i < road_centers.Num() - 1; i++)
		{
			...
			for (int j = 0; j < local_road.Num() - 1 && success_roads < 4; j++)
			{
				success_roads += create_guiding_road_segment(
					road_centers[i], local_road[j], true, EPointType::MainRoad, river);
			}
		}
	}
		...
		ECityPlan::radial_circle)
	{
		...
		auto trade_path1 = AllGeometry::create_segment_at_angle(
			...
			trade_path_angle, (map_params.x_size + map_params.y_size) / 2);
		auto trade_path2 = AllGeometry::create_segment_at_angle(
			left_node->get_FVector(), closest_center->get_FVector(),
			closest_center->get_FVector(),
			trade_path_angle + 180, (map_params.x_size + map_params.y_size) / 2);
		...
		for (int angle = trade_path_angle + 180 + FMath::RandRange(20, 30);
		     angle < trade_path_angle + 360 - 20; angle += FMath::RandRange(20, 30))
		{
			double dist_from_center = map_params.av_distance * (FMath::RandRange(0.25, 0.75));
			auto new_node_fvec = AllGeometry::create_segment_at_angle(
				left_node->get_FVector(), closest_center->get_FVector(),
		...
		for (int i = 0; i < local_road.Num() && success_roads <= 1; i++)
		{
			...
			local_road.Sort(
				[i, this, local_road](const TSharedPtr<Node>& Item1,
				                      const TSharedPtr<Node>& Item2)
				{
					return FVector::Distance(local_road[i]->get_FVector(), Item1->get_FVector()) < FVector::Distance(
						local_road[i]->get_FVector(), Item2->get_FVector());
				});
			for (int j = 0; j < local_road.Num(); j++)
			{
				success_roads += create_guiding_road_segment(
					local_road[i], local_road[j], false, EPointType::MainRoad, river);
...
bool TerrainGen::create_guiding_road_segment(
	...
	bool is_through_river, EPointType road_type, const TArray<TSharedPtr<Node>>& river)
{
	if (start_point == end_point ||
		(AllGeometry::is_intersect_array(start_point, end_point, river, true).IsSet() && !is_through_river))
	{
		return false;
	}
	...
	do
	{
		...
		if (inter_point1.IsSet())
		{
			...
			int angle = -90;
			if (!is_point_in_river(AllGeometry::create_segment_at_angle(point1->get_FVector(), point2->get_FVector(),
			                                                            inter_point1->Key, angle,
			                                                            0.1)))
			{
				angle = 90;
			}
			road_nodes.AddUnique(local_end_point);
			TSharedPtr<Node> inter_point2 = MakeShared<Node>(local_end_point->get_FVector());
			for (int i = 10; ; i += 5)
			{
			...
			real_bridge_end = AllGeometry::create_segment_at_angle(
				...
				next_start_point->get_FVector(), 0, map_params.bridge_indent);
			...
			FVector real_bridge_start = AllGeometry::create_segment_at_angle(
				...
				local_end_point->get_FVector(), 0, map_params.bridge_indent);
			local_end_point->set_FVector(real_bridge_start);
			next_start_point->set_FVector(real_bridge_end);
			create_segment(road_nodes, local_end_point, next_start_point, true,
			               road_type, (map_params.x_size + map_params.y_size) / 2);
		}

		TSharedPtr<Node> old_node = local_start_point;
		int dist_times = FVector::Distance(local_start_point->get_FVector(), local_end_point->get_FVector()) / (
		...
		for (int i = 1; i <= dist_times; i++)
		{
			...
			node->set_FVector(
				local_start_point->get_FVector() + ((local_end_point->get_FVector() - local_start_point->get_FVector())
					/ dist_times * i));
			if (i == dist_times)
			{
			...
			create_segment(road_nodes, old_node, node, true, road_type, map_params.max_road_length);
			old_node = node;
		}
	}
...
void TerrainGen::shrink_roads()
{
	...
	{
		for (int j = i + 1; j < road_nodes.Num(); j++)
		{
			if (FVector::Distance(road_nodes[i]->get_FVector(), road_nodes[j]->get_FVector())
				< map_params.min_road_length - 1)
			{
				add_conn(road_nodes[i], road_nodes[j]);
			}
		}
	}
	// for (auto& node : road_nodes)
	// {
	// 	if (node->get_node_type() != EPointType::Road && node->get_node_type() != EPointType::MainRoad && node->
	// 		get_node_type() != EPointType::Wall)
	// 	{
	// 		break;
	// 	}
	// 	for (auto& conn : node->conn)
	// 	{
	// 		if (conn->node->get_node_type() != EPointType::Road && conn->node->get_node_type() != EPointType::MainRoad
	// 			&& conn->node->get_node_type() != EPointType::Wall)
	// 		{
	// 			continue;
	// 		}
	// 		if (FVector::Distance(node->get_FVector(), conn->node->get_FVector()) > map_params.main_road_width)
	// 		{
	// 			del_conn(node, conn->node);
	// 		}
	// 	}
	// }
	...
	while (road_points != old_road_points)
	{
		...
		road_nodes.RemoveAll([&](const TSharedPtr<Node>& node)
		{
			...
			for (auto conn : node->conn)
			{
				if (FVector::Dist(node->get_FVector(), conn->node->get_FVector()) < map_params.min_road_length)
				{
			...
			EPointType::River)
		{
			...
			if (road_node->conn.Num() == 1)
			{
				if (FMath::FRand() * 100 <= map_params.road_forward_chance)
				{
					...
					double angle_in_degrees = FMath::RandRange(-map_params.segment_angle_deviation,
					                                           map_params.segment_angle_deviation);
					auto line1 = AllGeometry::create_segment_at_angle(road_node->conn[0]->node->get_FVector(),
					                                                  road_node->get_FVector(),
					                                                  road_node->get_FVector(), angle_in_degrees,
					                                                  length);

					TSharedPtr<Node> new_node = MakeShared<Node>(line1);
					new_node->set_type(EPointType::Road);
					...
					if (is_possible)
					{
						...
						auto finish = create_segment(
							add_road, road_node, new_node, false, EPointType::Road, map_params.max_road_length);
						if (finish.IsSet() && (finish.GetValue() != new_node || finish.GetValue()->get_node_type() ==
							EPointType::River))
						{
							is_end = true;
						}
			...
			if ((road_node->conn.Num() == 2 || road_node->conn.Num() == 1) && !is_end)
			{
				if (FMath::FRand() * 100 <= map_params.road_left_chance)
				{
					...
					new_node2->set_type(EPointType::Road);
					create_segment(add_road, road_node, new_node2, false,
					               EPointType::Road, map_params.max_road_length);
				}
				if (FMath::FRand() * 100 <= map_params.road_right_chance)
				{
					...
					new_node3->set_type(EPointType::Road);
	...
	double max_length)
{
	...
	do
	{
		...
		{
			if (intersection.GetValue()->get_node_type() == EPointType::Wall &&
				type != EPointType::Wall)
			{
			...
			to_exect_point)
		{
			...
			{
				end_point = insert_conn(
					presise_intersect->Value.Key,
	...
	while (FVector::Distance(end_point->get_FVector(), backup_endpoint->get_FVector()) > 0.0001);

	if (FVector::Distance(end_point->get_FVector(), backup_endpoint->get_FVector()) < 0.001)
	{
		end_point = backup_endpoint;
	}
	return end_point;
...
void TerrainGen::get_river_figure(const TArray<TSharedPtr<Node>>& river)
{
	...
	{
		item->set_district_type(Edistrict_type::Water);
	...
	// 		{
	// 			if (point.Value == Edistrict_type::Water && A->
...
void TerrainGen::process_districts(TArray<TSharedPtr<District>>& districts)
{
	...
	{
		// if (c.Value == Edistrict_type::Tower)
		// {
		debug2_points_array.Add(c.Key);
		// }

		if (is_point_in_river(c.Key) && c.Value != Edistrict_type::Water) continue;
		for (auto& d : districts)
		{
			if (d->get_district_type() != Edistrict_type::Unknown) continue;
			if (d->is_point_in_figure(c.Key))
			{
				if (c.Value == Edistrict_type::Royal)
				{
					royal_found = true;
				}
	...
	for (auto& b : districts)
	{
		...
		bool is_river = false;
		bool in_river = false;
		if (b->get_district_type() == Edistrict_type::Unknown)
		{
			...
			if (in_river)
			{
				b->set_district_type(Edistrict_type::Water);
				continue;
			}
		}
		if (b->get_district_type() == Edistrict_type::Unknown && !royal_found &&
			royal_area == 0)
		{
			bool point1 = false;
			int is_in_main = 0;
			for (auto& p : b->figure)
			{
				...
				if (p->get_node_type() == EPointType::MainRoad)
				{
					is_in_main++;
				}
				if (point1 && is_in_main >= 3)
				{
					b->set_district_type(Edistrict_type::Royal);
					royal_area += b->area;
					break;
				}
			}
		}
		if (b->get_district_type() == Edistrict_type::Unknown && royal_area > 0 && !
			royal_found)
		{
			for (auto p : b->figure)
			{
				...
				{
					if (district_near == Edistrict_type::Royal)
					{
						b->set_district_type(Edistrict_type::Royal);
						royal_area += b->area;
						break;
				...
				if (b->get_district_type() == Edistrict_type::Royal)
				{
					break;
				}
			}
		}
		if (b->get_district_type() == Edistrict_type::Royal && royal_area > 1000)
		{
			royal_found = true;
		}
		if (b->get_district_type() == Edistrict_type::Unknown)
		{
			...
			for (auto p : b->figure)
			{
				if (p->get_point()->districts_nearby.Contains(
					Edistrict_type::Royal))
				{
					is_near_royal = true;
				}
				if (p->get_point()->districts_nearby.Contains(
					Edistrict_type::Water))
				{
					is_near_dock = true;
				}
			}
			...
			{
				b->set_district_type(Edistrict_type::Dock);
			}
			if (is_near_royal && !is_near_dock && b->area > 6000)
			{
				b->set_district_type(Edistrict_type::Luxury);
			}
		}
	}
	...
	do
	{
		...
		for (auto& b : districts)
		{
			...
			{
				TMap<Edistrict_type, int32> ElementCount;
				...
				for (auto el : ElementCount)
				{
					switch (el.Key)
					{
					case Edistrict_type::Dock:
						{
							dock_count = el.Value;
							break;
						}
					case Edistrict_type::Royal:
						{
							royal_count = el.Value;
							break;
						}
					case Edistrict_type::Luxury:
						{
							luxury_count = el.Value;
							break;
						}
					case Edistrict_type::Residential:
						{
							residential_count = el.Value;
							break;
						}
					case Edistrict_type::Slums:
						{
							slums_count = el.Value;
							break;
						}
					case Edistrict_type::Tower:
					case Edistrict_type::Empty:
					case Edistrict_type::Unknown:
					case Edistrict_type::Water:
					default:
						break;
					}
				}
				...
				{
					b->set_district_type(Edistrict_type::Slums);
				}
				else if (koeff >= 0 && koeff < 4)
				{
					b->set_district_type(Edistrict_type::Residential);
				}
...
void TerrainGen::process_lights()
{
	...
	{
		if (street->get_type() == EPointType::MainRoad)
		{
	...
	{
		if (d->get_district_type() == Edistrict_type::Residential || d->get_district_type() == Edistrict_type::Luxury ||
			d->get_district_type() == Edistrict_type::Royal)
		{
			bool is_found = false;
			...
			for (auto& a : road_lantern_anchors)
			{
				...
				if (is_found)
				{
					PointObject l(closest_point, "Lantern");
...
{
	if (district->get_district_type() == Edistrict_type::Water)
	{
		return;
	}
	...
	if (district->get_district_type() == Edistrict_type::Luxury)
	{
		for (int i = 1; i <= 100; i++)
		{
	...
	else if (district->get_district_type() == Edistrict_type::Residential)
	{
	...
	else if (district->get_district_type() == Edistrict_type::Slums)
	{
		for (int i = 1; i <= 100; i++)
		{
			double angle = FMath::FRand() * 180;
...
void TerrainGen::create_special_district_by_nodes(TArray<TSharedPtr<Node>>& nodes, EPointType type, FVector point)
{
...
void TerrainGen::create_circle(FVector point, double radius, Edistrict_type type,
                               EPointType road_type, int vertex_count)
{
	custom_districts.Add(TTuple<FVector, Edistrict_type>(point, type));
...
void TerrainGen::create_circle_by_existing_nodes(FVector central_point, double radius, double interval,
                                                 Edistrict_type type, EPointType road_type, int vertex_count,
                                                 bool sticky_river, bool sticky_walls)
{
	custom_districts.Add(TTuple<FVector, Edistrict_type>(central_point, type));
...
{
	EPointType type_used;
	type_used = type != EPointType::Unidentified ? type : EPointType::Road;
	for (auto node : nodes)
	{
		for (auto nconn : node->conn)
		{
			...
			if (type != EPointType::Unidentified && (node->get_node_type() !=
			...
			street.set_street_type(type_used);
	...
	for (auto& street : fig_array)
	{
		...
		for (int i = 1; i < street->street_vertices.Num(); i++)
		{
			                                                       ...
			                                                       street_vertices.Num() - 1)
			{
				...
				cur_street->set_street_type(street->get_type());
