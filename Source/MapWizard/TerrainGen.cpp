                                ...
                                TArray<FVector>& debug2_points_array_)
{
	...
	point_type map_type = point_type::unidentified;
	if (water_type == EWaterType::island)
	{
	...
	double weight_width = water_type != EWaterType::island? 115:25;
	for (auto& r:river)
	{
		weighted_points.Add({r->get_FVector(), weight_width});
	}
...
void TerrainGen::create_rivers(TArray<WeightedPoint>& weighted_points, TArray<TSharedPtr<Node>>& river)
{
	...
	if (water_type == EWaterType::river)
	{
		...
		for (auto& w : weighted_points)
		{
			...
			else
			{
				float y1 = 25;
				float y2 = 115;
				float x1 = water_step * 2;
				float x2 = water_step * 3.5;
				float x0 = (x1 + x2) / 2;
				// this is hiperbole with middle inflex
				w.weight = ((y2 - y1) * (pow(smallest_dist - x0, 3) - pow(x1 - x0, 3))) / (pow(x2 - x0, 3) -
					pow(x1 - x0, 3)) + y1;
			}
		}
	}
	...
	for (auto& r : river)
	{
		for (int i = 0; i < 45; i++)
		{
			move_road(r, weighted_points, {map_borders_array, river}, water_step * 2);
		}
	}

	get_closed_figures(river, river_figures, 40);
	shapes_array = river_figures;
...
void TerrainGen::create_guiding_rivers(TArray<TSharedPtr<Node>>& river)
{
	...
	TSharedPtr<Node> start_point = MakeShared<Node>((start_river_point + end_river_point) / 2);
	start_point->set_type(point_type::river);

	FVector end_river =
		AllGeometry::create_segment_at_angle(FVector(0, 0, 0),
		                                     FVector{0, y_size, 0}, start_point->get_FVector(),
		                                     -90 + (FMath::FRand() * 20),
		...
		ECityPlan::radial_circle)
	{
		...
		for (auto r : road_centers)
		{
			if (FVector::Distance(r->get_FVector(), center) < closest_dist &&
				!is_point_in_river(r->get_FVector()) && !AllGeometry::is_point_near_nodes_array(
					river, r->get_FVector(), 200))
			{
		...
		for (int angle = trade_path_angle + 180 + FMath::FRand() * 10 + 20;
		     angle < trade_path_angle + 360 - 20; angle += FMath::FRand() * 10 + 20)
		{
...
void TerrainGen::shrink_roads(TArray<TSharedPtr<Node>>& road_nodes_array)
{
	...
	{
		old_road_points = road_nodes_array.Num();
		road_nodes_array.RemoveAll([&](const TSharedPtr<Node>& node)
		{
			if (node->conn.Num() < 2)
			{
				node->delete_me();
                                     ...
                                     TSharedPtr<Node> end_node, bool to_exact_point, point_type type, double max_length)
{
	...
	for (int i = 1; i < count; i++)
	{
		...
		if (!create_segment(array, old_node, next_node, true, type, max_length).IsSet())
			return false;
		old_node = next_node;
	}
...
void TerrainGen::point_shift(FVector& point, TArray<WeightedPoint> weighted_points)
{
	...
	weighted_points.Sort([&point](const WeightedPoint& A, const WeightedPoint& B)
	{
		return FVector::DistSquared(point, A.point) < FVector::DistSquared(point, B.point);
	});

	int Index = 0;
	for (int i = 0; i < weighted_points.Num(); i++)
	{
		if (FVector::Dist(point, weighted_points[i].point)>400)
		{
			Index = i-1;
			break;
		}
	...
	TArray<WeightedPoint> nearest_weighted_points;
	FVector backup = point;
	FVector result = weighted_points[0].point;
	// int N = weighted_points.Num();
	bool is_found = false;
	for (int32 i = 0; i < Index; i++)
	{
		for (int32 j = i + 1; j < Index; j++)
		{
			for (int32 k = j + 1; k < Index; k++)
			{
				if (AllGeometry::is_point_in_figure(point, {
					                                    weighted_points[i].point,
					                                    weighted_points[j].point,
					                                    weighted_points[k].point
				                                    }))
				{
					nearest_weighted_points.Add(weighted_points[i]);
					nearest_weighted_points.Add(weighted_points[j]);
					nearest_weighted_points.Add(weighted_points[k]);
					is_found = true;
					break;
				}
			}
	...
	if (!is_found) return;
	FVector direction = ((point - result) * weighted_points[0].weight).GetSafeNormal();
	result = weighted_points[1].point;
	direction += ((point - result) * weighted_points[1].weight).GetSafeNormal();
	result = weighted_points[2].point;
	direction += ((point - result) * weighted_points[2].weight).GetSafeNormal();

	backup += direction;
	if (FVector::Distance(backup, point) > 2)
	{
		point += ((backup - point)).GetSafeNormal() * 2;
	}
	else
	{
		point += direction;
	...
	for (auto node : nodes)
	{
		for (auto lconn : node->conn)
		{
			...
			while (second_node != node)
			{
				...
				{
					double angle = AllGeometry::calculate_angle(second_node->conn[i]->node->get_FVector(),
					                                            second_node->get_FVector(), first_node->get_FVector(),
					                                            true);

					if (angle < smallest_angle && angle > 0.01)
					{
...
void TerrainGen::get_river_figure(const TArray<TSharedPtr<Node>>& river)
{
	...
	// TODO: this is wrong, just for clarification
	get_closed_figures(river, river_fig_array, 40);
