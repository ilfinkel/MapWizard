...
{
	CustomDistrNodes(TSharedPtr<Node> node1_, TSharedPtr<Node> node2_, TSharedPtr<Node> node3_): node1(node1_)
	                                                                                           , node2(node2_)
	                                                                                           , node3(node3_)
	{
	};
...
public:
	...
	TerrainGen(FMapParams& map_params) : center(map_params.center)
	                                   ...
	                                   , max_river_length(map_params.max_river_length)
	                                   , min_new_road_length(map_params.min_new_road_length)
	                                   , min_road_length(map_params.min_road_length)
	                                   , av_road_length(map_params.av_road_length)
	                                   , max_road_length(map_params.max_road_length)
	                                   , river_road_distance(map_params.river_road_distance)
	...
	void get_closed_figures(TArray<TSharedPtr<Node>> nodes, TArray<TSharedPtr<District>>& fig_array, int figure_threshold);
